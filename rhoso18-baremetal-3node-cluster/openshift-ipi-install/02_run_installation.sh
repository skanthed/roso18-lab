...
cp -vf ${ASSETS_PATH}/install-config-template.yaml ${ASSETS_PATH}/install-config.yaml
...
cp -v ${ASSETS_PATH}/install-config.yaml ${CLUSTER_PATH}/install-config.yaml

${BIN_PATH}/openshift-baremetal-install --dir ${CLUSTER_PATH} --log-level debug create manifests

#cp -v ${ASSETS_PATH}/MC/* ${CLUSTER_PATH}/openshift/
